...
var ENTITY_VENDOR='vendor';
var ENTITY_ITEM='item';
var ENTITY_ORDER='order';
var ENTITY_REPORT='report';
...
var add = function(entity) {
	...
	$("select[id$=item-vendor-list] > option").remove();
	//checking the entity to populate the select box
	if (entity == ENTITY_ITEM) {
		//populating the vendor and contact by making an ajax call
		populateSelectBox('item-vendor-list', '/vendor');
...
var showMessage = function(message, entity){
	$('#'+entity+'-show-message').show().html('<p><img src=../images/error.jpg/><b>'+message+'</b><br></p>');
}
...
var formValidate = function(entity){
	...
	switch(entity){
		case ENTITY_ITEM:
			var valueProduct = $('#item-vendor-list').val();
			if(valueProduct == "" || key == ""){
		...
		if(data!=null && data.length > 0){
			for (var i=0;i<data.length;i++){
				...
				case ENTITY_VENDOR:
					...
					htm+='<td>'+data[i].nbr+'</td><td>'+data[i].name+'</td><td>'+data[i].address+'</td><td>'+data[i].city+'</td><td>'+data[i].st+'</td><td>'+data[i].zip+'</td><td>'+data[i].phn+'</td><td>'+data[i].email+'</td><td>'+data[i].rep+'</td>';
					break;
				case ENTITY_ITEM:
					
					htm+='<td>'+data[i].vendor+'</td><td>'+data[i].name+'</td><td>'+data[i].price+'</td><td>'+data[i].quantity+'</td><td>'+data[i].desc+'</td><td>'+data[i].cost+'</td><td>'+data[i].min_level+'</td><td>'+data[i].status+'</td>';
					break;
				case ENTITY_ORDER:
				...
				case ENTITY_CUSTOMER:
					...
					//htm+='<td>'+data[i].name+'</td><td>'+data[i].firstName+'</td><td>'+data[i].lastName+'</td><td>'+data[i].suffix+'</td><td>'+data[i].address+'</td><td>'+data[i].city+'</td><td>'+data[i].state+'</td><td>'+data[i].zip+'</td><td>'+data[i].phone+'</td><td>'+data[i].eMail+'</td>';
					htm+='<td class=results-bg>'+data[i].firstName+'</td><td>'+data[i].lastName+'</td><td>'+data[i].suffix+'</td><td>'+data[i].zip+'</td><td>'+data[i].eMail+'</td>';
					break;
				case ENTITY_REPORT:
