...
Entity Donation = (Entity)request.getAttribute("MoneyDonation");
NodeList xmlPerson=(NodeList)request.getAttribute("xmlPerson");
NodeList xmlAddress=(NodeList)request.getAttribute("xmlAddress");
NodeList xmlDonation=(NodeList)request.getAttribute("xmlMoneyDonation");
    ...
    <tr>
      <td class="content-body">
        <div class="content">
          ...
          <br/>
          <!----Search Form ------->
          <form method="POST" action="<%=request.getContextPath()%>/dispatch?page=moneyDonation" id="dispatchForm">
              ...
              <tr>
                <td>
                  <table width="500" cellspacing="0" cellpadding="0" border="0" style="border-collapse:collapse;">
		             ...
		             -->
                    <%
                    for (int i=0;i<xmlDonation.getLength();i++) {
                      Element propertyElement = (Element)xmlDonation.item(i);
                      String name=propertyElement.getAttribute("name").toString();
                      String type=propertyElement.getAttribute("type").toString();
                      String edit=propertyElement.getAttribute("editable").toString();
                      String maxlength=propertyElement.getAttribute("maxlength").toString();
                            ...
                            <td class="<% if (i%2==0) { %>edit-value-dark<%}else{%>edit-value-white<%}%>" >
                              ...
                              if ("Y".equals(edit)) {
                                ...
                                if ("text".equals(input)) {
                                  ...
                                  <input 
                                      name="<%=name%>"
                                      value="<%=Donation.getProperty(propertyElement.getAttribute("name").toString())%>"
                                      maxlength="<%=maxlength%>"
                                      size="<%=maxlength%>"
                                      class="entry"/>
                                  <%
                                ...
                                if ("calendar".equals(input)) {
                                    ...
                                    if (ddl!=null) {
                                      for (int j=0;j<ddl.size();j++) {
                                        ...
                                        <option 
                                            ...
                                            <%
                                            if
                                            (ddl.get(j).equals(Donation.getProperty(propertyElement.getAttribute("name").toString())))
                                            {%>selected<%}%>>
                                  ...
                                  %>
                                  <textarea name="<%=name%>" class="dispatch"><%=Donation.getProperty(propertyElement.getAttribute("name").toString())%></textarea>
                                  <%
                                }

                              } else {
                                %>
                                <%=Donation.getProperty(propertyElement.getAttribute("name").toString())%>
                                <%
                              }
                              %>
                            </td>
                      ...
                      <tr>
                        ...
                        <td height="50" valign="center">
                          ...
                          <select name="farmSource" class="entry">
                            ...
                            if (ddl!=null) {
                              String farmSource="";
                              if (Donation.getProperty("farmSource")!=null)
                              farmSource=Donation.getProperty("farmSource").toString();
